...
gulp.task('scripts', function() {
    var scripts = [
        ...
        basePaths.dev + 'js/jquery.easeScroll.js',
    ...
    var adminScripts = [
        basePaths.dev + 'js/admin/pt-admin-main.js',
    ];

    gulp.src(adminScripts)
        .pipe(concat('pt-theme-admin.min.js'))
        .pipe(uglify())
        .on('error', function (err) { gulpUtil.log(gulpUtil.colors.red('[Error]'), err.toString()); })
        .pipe(gulp.dest('./js/admin/'));

    gulp.src(adminScripts)
        .pipe(concat('pt-theme-admin.js'))
        .pipe(gulp.dest('./js/admin/'));
});
...
// Copy jQuery Masonry Layout
    gulp.src(basePaths.node + 'masonry-layout/dist/*.min.js')
        .pipe(gulp.dest(basePaths.dev + '/js'));
